...
const PAGE_META = {
  ...
  pilotage: ["Pilotage DFY", "Onboarding, P&L, rapport hebdo — paiement fournisseur manuel"],
...
async function loadPilotage() {
  const [pnlRes, obRes, reportRes] = await Promise.all([
    fetch("/api/ops/pnl").then((r) => r.json()),
    fetch("/api/ops/onboarding").then((r) => r.json()),
    fetch("/api/ops/weekly-report").then((r) => r.json()),
  ]);
  const pnl = pnlRes.data || {};
  const ob = obRes.data?.onboarding || {};
  const next = obRes.data?.nextStep || {};
  const gmvEl = document.getElementById("ops-gmv");
  if (!gmvEl) return;
  gmvEl.textContent = `${pnl.gmv ?? "—"} €`;
  document.getElementById("ops-net").textContent = `${pnl.net ?? "—"} € (${pnl.netPct ?? 0} %)`;
  document.getElementById("ops-pending").textContent = String(pnl.pendingOrders ?? "—");
  document.getElementById("ops-next").textContent = next.label || "—";
  const labels = {
  ...
  document.getElementById("ops-onboarding").innerHTML = Object.keys(labels)
    .map(
      (k) =>
        `<label class="flex items-center gap-2"><input type="checkbox" data-ops="${k}" ${ob[k] ? "checked" : ""}/> ${labels[k]}</label>`
    )
    .join("");
  const actions = reportRes.data?.actions || [];
  document.getElementById("ops-actions").innerHTML = actions.map((a) => `<li>${escapeHtml(a.text)}</li>`).join("");
}

async function savePilotageOnboarding() {
  const payload = {};
  document.querySelectorAll("#ops-onboarding input[data-ops]").forEach((el) => {
    payload[el.dataset.ops] = el.checked;
  });
  await fetch("/api/ops/onboarding", {
    method: "POST",
    headers: { "Content-Type": "application/json" },

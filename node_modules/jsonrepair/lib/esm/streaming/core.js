...
export function jsonrepairCore(_ref) {
  ...
  function parseString() {
    ...
    if (isQuote(input.charAt(i)) || openedByEntity) {
      ...
      while (true) {
        ...
        const entity = openedByEntity && input.charAt(i) === '&' ? matchHtmlEntity(htmlEntityWindow(i)) : null;
        ...
        if (isEnd) {
          ...
          if (stopAtDelimiter || input.isEnd(i) || isDelimiter(input.charAt(i)) &&
          ...
          !isInsideUnclosedBracket(input.substring(iBefore, iQuote), input.charAt(i)) || isQuote(input.charAt(i)) && !nextQuoteIsEndQuote(i) || isDigit(input.charAt(i))) {
  ...
  function parseKeyword(name, value) {
    if (input.substring(i, i + name.length) === name && !isFunctionNameChar(input.charAt(i + name.length))) {
      output.push(value);
      i += name.length;
      return stack.update(Caret.afterValue);
    }

...
function jsonrepairCore(_ref) {
   ...
   */
  function htmlEntityWindow(at) {
    const max = Math.min(at + _stringUtils.maxHtmlEntityLength, input.currentLength());
  ...
  function parseString() {
    ...
    const openEntity = input.charAt(i) === '&' ? (0, _stringUtils.matchHtmlEntity)(htmlEntityWindow(i)) : null;
    const openedByEntity = (0, _stringUtils.isDoubleQuoteEntity)(openEntity) || (0, _stringUtils.isSingleQuoteEntity)(openEntity);
    if ((0, _stringUtils.isQuote)(input.charAt(i)) || openedByEntity) {
      ...
      while (true) {
        ...
        const entity = openedByEntity && input.charAt(i) === '&' ? (0, _stringUtils.matchHtmlEntity)(htmlEntityWindow(i)) : null;
        const isEnd = entity && openEntity ? entity.char === openEntity.char : isEndQuote(input.charAt(i));
        if (isEnd) {
          // end quote
          ...
          if (stopAtDelimiter || input.isEnd(i) || (0, _stringUtils.isDelimiter)(input.charAt(i)) &&
          ...
          !(0, _stringUtils.isInsideUnclosedBracket)(input.substring(iBefore, iQuote), input.charAt(i)) || (0, _stringUtils.isQuote)(input.charAt(i)) && !nextQuoteIsEndQuote(i) || (0, _stringUtils.isDigit)(input.charAt(i))) {
  ...
  function parseNumber() {
    ...
    if (input.charAt(i) === '-') {
      num += input.charAt(i);
      i++;
      if (!(0, _stringUtils.isDigit)(input.charAt(i)) && atEndOfNumber()) {
        num += '0';
      }
    }
    if (input.charAt(i) === '0' && (0, _stringUtils.isDigit)(input.charAt(i + 1))) {
  ...
  function parseKeyword(name, value) {
    if (input.substring(i, i + name.length) === name && !(0, _stringUtils.isFunctionNameChar)(input.charAt(i + name.length))) {
      output.push(value);
      i += name.length;
      return stack.update(_stack.Caret.afterValue);
    }
  ...
  function nextQuoteIsEndQuote(index) {
    ...
    let next = index + 1;
    while (!input.isEnd(next) && (0, _stringUtils.isWhitespace)(input, next)) {
      next++;
    }
    return input.isEnd(next) || (0, _stringUtils.isDelimiter)(input.charAt(next));
  }

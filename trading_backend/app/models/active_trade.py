...
class ActiveTrade(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "capital_engaged >= 0",
            name="ck_active_trades_non_negative_capital",
        ),
        CheckConstraint(
            "estimated_duration_minutes > 0",
            name="ck_active_trades_positive_duration",
        ),
        CheckConstraint(
            "probability_used >= 0 AND probability_used <= 100",
            name="ck_active_trades_probability_percent",
        ),
        CheckConstraint(
            "status IN ('open', 'closed')",
            name="ck_active_trades_status",
        ),
    )
